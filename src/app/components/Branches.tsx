  ...
  return <section className="container mx-auto mt-8 border-t border-zinc-200 px-4 pt-8">
    ...
    <div className="grid gap-6 md:grid-cols-3">
      {stores.filter(store => store.isActive).map(store => <div key={store.id} className="border-l border-zinc-200 pl-4 text-zinc-700">
        ...
        <a href={googleMapsUrl(store.address, store.city, store.state, store.googleMapsUrl)} target="_blank" rel="noopener noreferrer" className="mt-2 flex items-start gap-2 hover:underline"><MapPin className="h-4 w-4 shrink-0" />{store.address}, {store.city}, {store.state}</a>
